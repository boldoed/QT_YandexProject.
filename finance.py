...
class AddRashod(QDialog):
    def __init__(self):
        ...
        self.rashod_window.itemDoubleClicked.connect(self.delete_func)
        self.update_rashod()
    ...
    '''
    добовляет элемент в список расходов
    '''
    def add_new_rashod(self):
        self.rashod_spisok = []
        self.spisok_rashodov = self.cursor.execute("""SELECT * FROM rashodi""").fetchall()
        elem = self.spisok_rashodov[-1][1:]
        elem = ', '.join([str(i) for i in elem])
        self.rashod_window.addItem(elem)
